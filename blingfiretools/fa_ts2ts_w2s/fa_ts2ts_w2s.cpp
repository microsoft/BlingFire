...
void usage () {
  ...
  std::cout << "\n\
Usage: fa_ts2ts_w2s [OPTIONS] [< input.utf8] [> output.utf8]\n\
\n\
This program splits compound words (using W2S) in the tagged text. Word text\n\
...
int __cdecl main (int argc, char ** argv)
{
    ...
    try {
        ...
        // W2S interpreter
        FAPrmInterpreter_t < int > g_prm (&g_alloc);
        // input tagged text
        FATaggedText g_text (&g_alloc);
        FATaggedText g_text_w2s (&g_alloc);
        ...
        while (!g_pIs->eof ()) {
            ...
            int pEnds [FALimits::MaxWordLen];

            // split compounds
            const int Count = g_text.GetWordCount ();

            for (int i = 0; i < Count; ++i) {

                const int * pWord;
                const int Size = g_text.GetWord (i, &pWord);
                const int Tag = g_text.GetTag (i);
                const int Offset = g_text.GetOffset (i);

                const int EndsCount = g_prm.ProcessW2S (pWord, Size, \
                    pEnds, FALimits::MaxWordLen);

                // not a compound ?
                if (1 >= EndsCount) {

                    g_text_w2s.AddWord (pWord, Size, Tag, Offset);

                // compound
                } else {

                    int BeginPos = 0;
                    for (int j = 0; j < EndsCount; ++j) {

                        const int EndPos = pEnds [j];
                        const int SegSize = EndPos - BeginPos + 1;
                        DebugLogAssert (0 < SegSize && SegSize <= Size);

                        g_text_w2s.AddWord (pWord + BeginPos, SegSize, \
                            Tag, Offset + BeginPos);
            ...
            g_txt_io.Print (*g_pOs, &g_text_w2s);
